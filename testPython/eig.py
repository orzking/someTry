...
def larger(a,b):
    if a>b:
        ...
        return b
#根据特征值获取id 和相似度
def getIdByData(iddatafile,data,threshold):
    #TODO:
    similarity=0.0
    id=None
    if(False==os.path.exists(iddatafile)):
...
if __name__=='__main__':
    ...
    put_idname(idNameFile,idname3)
    
    print(get_name(idNameFile,'002'))
    idname1={'id':'002','name':'李白'}
    put_idname(idNameFile,idname1)
    print(get_name(idNameFile,'002'))
    id,similarity=getIdByData(idDataFile,data12,0.45)
    print('id=',id,',similarity=',similarity)
    print('匹配到：',get_name(idNameFile,id))
